<?php
/**
 * Tests for the Enqueued_Styles_Size_Check class.
 *
 * @package plugin-check
 */

use WordPress\Plugin_Check\Checker\Checks\Performance\Enqueued_Styles_Size_Check;
use WordPress\Plugin_Check\Checker\Preparation;
use WordPress\Plugin_Check\Test_Utils\TestCase\Runtime_Check_UnitTestCase;

class Enqueued_Styles_Size_Check_Tests extends Runtime_Check_UnitTestCase {

	public function test_get_shared_preparations() {
		$check        = new Enqueued_Styles_Size_Check();
	...
	public function test_run_without_errors() {
		...
		require UNIT_TESTS_PLUGIN_DIR . 'test-plugin-enqueued-style-size-check/load.php';
	...
	public function test_run_with_errors() {
		...
		$check   = new Enqueued_Styles_Size_Check( 1 );
		$context = $this->get_context( WP_PLUGIN_CHECK_MAIN_FILE );
		$results = $this->run_check( $check, $context );
	...
	public function test_run_with_errors_for_inline_style() {
		// Load the test plugin.
		require UNIT_TESTS_PLUGIN_DIR . 'test-plugin-enqueued-style-size-check/load.php';

		// Test with threshold under the enqueued test-style.css byte size.
		$check   = new Enqueued_Styles_Size_Check( 20 );
		$context = $this->get_context( WP_PLUGIN_CHECK_MAIN_FILE );
		$results = $this->run_check( $check, $context );
