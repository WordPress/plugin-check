 ...
 *
 * Feature tests context class with plugin-specific steps.
 */

namespace WordPress\Plugin_Check\Behat_Utils;
...
final class FeatureContext extends WP_CLI_FeatureContext {
	...
	/**
	 * @Given a WP install(ation) with the Plugin Check (plugin)
	 */
	...
	public function when_i_run_the_wp_cli_command( $mode, $command ) {
		...
		if ( \in_array( $with_code_coverage, [ true, 'true', 1, '1' ], true ) ) {
			$command = "{$command} --require={PROJECT_DIR}/tests/behat/utils/maybe-generate-wp-cli-coverage.php";
		}
